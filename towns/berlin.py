...
# get Berlin values
df_2019_Berlin = filter_state(statedict["Berlin"], df_2019)
df_2018_Berlin = filter_state(statedict["Berlin"], df_2018)

#filter for berlin, AGS = 11 0 00 000 BL Regbez Kreis Gemeinde
...
df_2019_Berlin_berlin = filter_town_ags('11000000', df_2019_Berlin)
df_2018_Berlin_berlin = filter_town_ags('11000000', df_2018_Berlin)
...
df_2019_Berlin_berlin_bike = filter_bike(df_2019_Berlin_berlin)
df_2018_Berlin_berlin_bike = filter_bike(df_2018_Berlin_berlin)
...
center = findcenter(df_2019_Berlin_berlin_bike, 'YGCSWGS84', 'XGCSWGS84')
print(center)
...
df_list = [(df_2019_Berlin_berlin_bike, YearColor.color2019),
            (df_2018_Berlin_berlin_bike, YearColor.color2018)
            ]
...
def save_map(name):
    tic = time.perf_counter()
    map.save("../html/berlin/" + name+".html")
...
debug = False
if debug == True:
    print((df_2019_Berlin['UREGBEZ']))
    print((df_2019_Berlin['UKREIS']))
    print(df_2019)
    print(df_2019_Berlin_berlin)
    print(df_2019_Berlin_berlin_bike)
    print(center)

casualties = True
if casualties == True:
    dead = []
    dead.append((str(df_2019_Berlin_berlin_bike.loc[df_2019_Berlin_berlin_bike['UKATEGORIE'].astype(int) == 1].shape[0]), '2019'))
    dead.append((str(df_2018_Berlin_berlin_bike.loc[df_2018_Berlin_berlin_bike['UKATEGORIE'].astype(int) == 1].shape[0]), '2018'))
    
    h_injured = []
    h_injured.append((str(df_2019_Berlin_berlin_bike.loc[df_2019_Berlin_berlin_bike['UKATEGORIE'].astype(int) == 2].shape[0]), '2019'))
    h_injured.append((str(df_2018_Berlin_berlin_bike.loc[df_2018_Berlin_berlin_bike['UKATEGORIE'].astype(int) == 2].shape[0]), '2018'))
    
    l_injured = []
    l_injured.append((str(df_2019_Berlin_berlin_bike.loc[df_2019_Berlin_berlin_bike['UKATEGORIE'].astype(int) == 3].shape[0]), '2019'))
    l_injured.append((str(df_2018_Berlin_berlin_bike.loc[df_2018_Berlin_berlin_bike['UKATEGORIE'].astype(int) == 3].shape[0]), '2018'))
